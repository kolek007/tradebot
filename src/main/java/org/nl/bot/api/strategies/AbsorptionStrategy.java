        ...
        public void onEvent(CandleEvent candleEvent) {
            ...
            if(first == null) {
                ...
                if(green(second) && meetsRequirements(second) && weakEqual(firstHeight, secondHeight, ERROR_PERCENTAGE)) { //main strategy condition met, need to buy
                    ...
                    adapter.placeOrder(getId(), instrument.getTicker(), new OrderTkf(1, Operation.Sell, takeProfit), null); //TAKE PROFIT
                    adapter.placeOrder(getId(), instrument.getTicker(), new OrderTkf(1, Operation.Sell, stopLoss), null); //STOP LOSS
                }
