...
public class AbsorptionStrategy extends AbstractStrategy {
        ...
        public void onEvent(CandleEvent candleEvent) {
            ...
            if(first == null) {
                ...
                if(green(second) && meetsRequirements(second) && weakEqual(firstHeight, secondHeight, ERROR_PERCENTAGE)) { //main strategy condition met, need to buy
                    ...
                    adapter.placeOrder(getId(), instrument.getTicker(), createOrder(1, Operation.Buy, price), null).join(); //TODO need to think about price and lots amount
                    adapter.placeOrder(getId(), instrument.getTicker(), createOrder(1, Operation.Sell, takeProfit), null); //TAKE PROFIT
                    adapter.placeOrder(getId(), instrument.getTicker(), createOrder(1, Operation.Sell, stopLoss), null); //STOP LOSS
                }
    ...
    @Nonnull
    private Order createOrder(int lots, @Nonnull Operation operation, @Nonnull BigDecimal price) {
        return new Order() {
            @Override
            public int getLots() {
                return lots;
